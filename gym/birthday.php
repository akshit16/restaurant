...
<?php 
        ...
        $deposit = $row[17];
				...
				$dob= $row['dob'];
				//$assign = $row[6];
				$m = date("F",strtotime($dob));
				$d= date("d",strtotime($dob));
				$mt = date("F",strtotime(time()));
				$dt = date("Y",strtotime(time()));
				if($d==$dt&&$mt=$m){
				echo "
                        <tr>
                          <td>
				...
				echo '
                          ...
                          <td>'.$dob.'</td>
                          <td data-order="'.$dt.'">
                            <span class="label label-info label-pill">Rs '.$due.'</span>
                          </td>' ;

                           if(time() > strtotime($duedate)-(60*60*24*10))

                           echo ' <td style ="color:red;" data-order="'.$duedate.'">'.$duedate.'</td>';
                          else
                            echo ' <td  data-order="'.$duedate.'">'.$duedate.'</td>';
                            echo '<td data-order="'.$batch.'">'.$batch.'</td>
                          <td>
