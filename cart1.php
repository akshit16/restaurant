...
$dish = $row['Dish'];
$price = $row['Price'];
$sql1= "select * from cart where User = '{$_SESSION["user"]}' and Dish = '$dish'";
...
values ('{$_SESSION["user"]}','$dish',$price,$qty,$sum)";
...
{
$up = "update cart set quantity = $qty where user = '{$_SESSION["user"]}' and Dish = '$dish'";
...
$tum = $qty*$price;
 $dis = "update cart set total = $tum where user ='{$_SESSION["user"]}' and Dish='$dish'";
                 $rs = mysqli_query($conn,$dis);
                 $sel = "select total from cart where user ='{$_SESSION["user"]}'";
